...
def replace_function(content: str) -> str:
    # 匹配 extract_twitter_fallback_streams 函数（从 def 到 return 行）
    pattern = r'(def extract_twitter_fallback_streams\(html: str\) -> list\[str\]:\s*patterns = \[[\s\S]*?return dedupe_keep_order\(cleaned\))'
    new_func = '''def extract_twitter_fallback_streams(html: str) -> list[str]:
    patterns = [
    ...
    return re.sub(pattern, new_func, content, count=1, flags=re.DOTALL)
