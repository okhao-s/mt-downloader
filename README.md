...
- 回调请求仍然快速返回，避免企业微信超时
- 后台继续异步通过企业微信应用消息主动回文本：真实任务创建后发一条“带平台前缀 + 文件名”的创建回执；任务状态进入 `done / failed / cancelled` 时，直接绑定真实 job 状态变更发最终通知
...
- 创建任务时：真实 job 创建后主动回一条简洁通知，格式统一为 `[平台] 已收到链接，正在创建任务：抓取到的文件名`
