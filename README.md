...
- 复用现有下载主链创建任务
- 文本消息先走**被动回复**：即时回一条“已收到链接，正在创建任务”，保证用户立刻看得到回执
- 回调请求仍然快速返回，避免企业微信超时
- 后台继续异步通过企业微信应用消息主动回文本：真实 job 写入网页任务列表后发“任务创建成功”通知；任务状态进入 `done / failed / cancelled` 时，直接绑定真实 job 状态变更发最终通知
...
- 创建任务时：真实 job 创建并写入网页任务列表后主动回一条简洁通知，格式统一为 `[平台] 任务创建成功：抓取到的媒体名`
- 任务结束后：在真实 job 状态进入 `done / failed / cancelled` 时立即主动回最终结果，比如 `[Douyin] 下载完成 / 下载失败 / 任务已取消`，并附标题或文件名、任务ID；失败时会附简短原因
