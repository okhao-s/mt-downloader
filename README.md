...
- 回调请求仍然快速返回，避免企业微信超时
- 后台继续异步通过企业微信应用消息主动回文本：创建时发一条“带平台前缀的任务创建回执”，任务结束后再主动回“带平台前缀的下载完成 / 下载失败 / 已取消”
...
- 创建任务时：发一条合并回执，只在原通知前面加平台前缀，比如 `[Douyin] 已识别链接，开始解析并创建任务...`，同条消息里带任务ID、标题/文件名、当前排队状态
- 任务结束后：再主动回最终结果，比如 `[Douyin] 下载完成 / 下载失败 / 任务已取消`，并附标题或文件名、任务ID；失败时会附简短原因
