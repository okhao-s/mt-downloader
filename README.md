...
- 异步通过企业微信应用消息主动回文本：会先回“平台识别 / 已创建任务”，任务结束后再主动回“下载完成 / 下载失败 / 已取消”
...
- 收到链接后：先回平台识别结果，比如 `已识别为 Douyin / YouTube / Bilibili 任务`
- 创建任务后：回任务已入队，附平台、任务ID、标题/文件名、当前排队状态
- 任务结束后：再主动回最终结果，包含完成/失败/取消、平台、标题或文件名、任务ID；失败时会附简短原因
