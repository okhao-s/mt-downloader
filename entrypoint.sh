...
if [ -f "$CONFIG_FILE" ]; then
  HTTP_PROXY_VAL=$(python3 -c "import json; c=json.load(open('$CONFIG_FILE')); print(c.get('http_proxy','') or c.get('default_proxy','') or '')" 2>/dev/null || true)
  HTTPS_PROXY_VAL=$(python3 -c "import json; c=json.load(open('$CONFIG_FILE')); print(c.get('https_proxy','') or c.get('default_proxy','') or '')" 2>/dev/null || true)
